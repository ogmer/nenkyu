...
// クライアント側の再検証用フォールバック（初回はサーバー(ISR)の値を使用するため通常は実行されない）
...
export function HolidayCalculatorClient({ initialHolidayCount }: { initialHolidayCount: number }) {
  const [workingDaysPerWeek, setWorkingDaysPerWeek] = useState("5")
  const [nationalHolidays, setNationalHolidays] = useState(initialHolidayCount.toString())
  ...
  // サーバー(ISR)で取得した祝日数をinitialDataとして利用。再検証はTanStack Queryが担う
  const { data: fetchedHolidayCount } = useQuery({
    queryKey: ["weekday-holidays", CURRENT_YEAR],
    queryFn: fetchWeekdayHolidayCount,
    initialData: initialHolidayCount,
  })
  ...
  const shareOnTwitter = useCallback(() => {
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      navigator.share({ text, url: window.location.href }).catch(() => openShareUrl(shareUrl))
      return
    }
              ...
              <Card className="mb-8">
                ...
                <CardContent className="space-y-6">
                  ...
                  <MemoizedInput
                    ...
                    helpText="今年の平日の祝日数が設定されています（土日重複分は除外済み）"
                  />
